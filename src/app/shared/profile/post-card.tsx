...
export default function PostCard({ type, thumbnail }: PostCard) {
  return (
    <div className="group group relative aspect-square h-full w-full cursor-pointer overflow-hidden bg-gray-100">
      ...
      {type === 'video' && (
        <PiVideoCameraFill className="absolute left-4 top-4 z-10 h-8 w-8 rounded-full bg-gray-200 p-2 text-xl text-gray-1000" />
      )}

      {type === 'gallery' && (
        <PiImagesSquareBold className="absolute left-4 top-4 z-10 h-8 w-8 rounded-full bg-gray-200 p-2 text-xl text-gray-1000" />
      )}
    </div>
  );
}
