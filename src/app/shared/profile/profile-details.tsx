...
const tabs = [
  { id: 'posts', count: postData.length, name: 'پست ها' },
  { id: 'followers', count: followersData.length, name: 'دنبال کنندگان' },
  { id: 'following', count: followingData.length, name: 'دنبال شوندگان' },
];
...
export default function ProfileDetails() {
  ...
  function handleTabClick(id: string) {
    if (id === 'followers') {
      setModalData({ title: 'دنبال کنندگان', data: followersData });
    } else if (id === 'following') {
      setModalData({ title: 'دنبال شوندگان', data: followingData });
    }
    setOpen(() => true);
    setActive(() => id);
  }
