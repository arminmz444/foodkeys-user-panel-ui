...
export default function MessageBody({messages}) {
    ...
    return (
        messages && messages.length > 0 ? messages.map((message: {
            fromEmployee: any;
            id: boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | PromiseLikeOfReactNode | Key | null | undefined;
            senderAvatar: string;
            senderName: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined;
            senderUsername: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined;
            createdAtStr: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined;
            messageContent: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined;
            ticketId: string | number | boolean | ReactElement<any, string | JSXElementConstructor<any>> | Iterable<ReactNode> | ReactPortal | PromiseLikeOfReactNode | null | undefined;
            attachments: any[];
        }) => {
            ...
            return (
                <div key={message.id}>
                    <div className={`grid items-start gap-3 lg:gap-4 ${avatarAlignmentClass}`}>
                        ...
                        {/*    className="!h-8 !w-8 bg-[#70C5E0] font-medium text-white xl:!h-11 xl:!w-11"*/}
                        {/*/>*/}
                        <div className={`-mt-1.5 lg:mt-0 ${message.fromEmployee ? "" : "order-2"}`}>
            ...
            <span className="flex items-center lowercase">
              {message?.senderUsername} <FiExternalLink className="mr-1 h-2.5 w-2.5"/>
                                ...
                                <span className="mt-1.5 flex items-center lg:mt-0">
                                    ...
                                    >
                                                    {/*// @ts-ignore*/}
                                        <button type="button" onClick={() => handleCopyToClipboard(message?.id)}>
                  {isCopied ? (
                      ...
                      <HiOutlineClipboardDocument className="mr-1 h-3 w-3"/>
                        ...
                        {message.fromEmployee && (
                            <Avatar
                                name="John Doe"
                                src={STATIC_FILES_URL + message?.senderAvatar}
                                className="!h-8 !w-8 bg-[#70C5E0] font-medium text-white xl:!h-11 xl:!w-11 order-1 lg:order-2"
                            />
                        )}
                    </div>
                    ...
                    <div className="ml-10 mt-3 grid gap-2 leading-relaxed xl:ml-16 2xl:mt-4">
                        ...
                        {!isEmpty(message?.attachments) && (
                            <div className="mt-2 grid gap-2 md:grid-cols-2 lg:grid-cols-1 2xl:grid-cols-3">
                                {/* @ts-ignore */}
                                {message?.attachments.map((attachments: string[]) => (
                                    <div
                                        // @ts-ignore
                                        key={attachments.id}
                                        ...
                                        <figure className="relative h-10 w-10 overflow-hidden rounded">
                                            ...
                                            {attachments.type === 'image' ? (
                                                <Image
                                                    ...
                                                    // @ts-ignore
                                                    src={STATIC_FILES_URL + attachments.filePath}
        ...
        }) : <center><Text>پیامی وجود ندارد</Text></center>
    );
}
