...
export default function CompanyFactory({
  ...
  useEffect(() => {
    ...
    if (watchedContacts && watchedContacts.length) {
  ...
  useEffect(() => {
    ...
    const formattedTels = factoryTels.map((tel, index) => ({
      id: index,
      telType: 'FACTORY',
      telNumber: tel,
      companyId: data?.id || 0,
    }));
    setValue('factoryTels', formattedTels);
  }, [factoryTels, setValue, data?.id]);

  useEffect(() => {
    // Convert string array to required format for the form
    const formattedFaxes = factoryFaxes.map((fax, index) => ({
      id: index,
      telType: 'FACTORY_FAX',
      telNumber: fax,
      companyId: data?.id || 0,
    }));
    setValue('factoryFaxes', formattedFaxes);
  }, [factoryFaxes, setValue, data?.id]);
          ...
          {factoryFaxes.map((fax, index) => (
              <div key={index} className="flex items-center gap-2 space-x-2">
                <Input
                    ...
                    onChange={(e) => {
                      const newFaxes = [...factoryFaxes];
                      newFaxes[index] = e.target.value;
                      setFactoryFaxes(newFaxes);
                    }}
                    className="flex-grow"
                />
                <ActionIcon
