...
export default function CompanySummary({className, category}: { className?: string, category: number }) {
    // const { data: subcategories, isLoading, error } = useQuery('subcategories', fetchSubcategories);
    const _axios = useAxiosPrivate()
    const [subcategories, setSubcategories] = useState<{ value: number; name: string }[]>([]);
    const [companyTypeOptions, setCompanyTypeOptions] = useState<{ value: string; name: string }[]>([]);
        ...
        >
            ...
            <Controller
                ...
                render={({field: {onChange, value, onBlur}}) => (
                    // @ts-ignore
                    <JalaliDatePicker selected={value}
                        onChange={onChange}
                        onBlur={onBlur}
