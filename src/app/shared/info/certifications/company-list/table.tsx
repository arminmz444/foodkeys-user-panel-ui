...
const fetchCompanies = async (searchTerm: string, currentPage: number, pageSize: number) => {
    const API_URL = searchTerm
        ? `https://foodkeys-api-dev.liara.run/api/v1/search/company?query=${searchTerm}&page=${currentPage}&size=${pageSize}`
        : `https://foodkeys-api-dev.liara.run/api/v1/company/?pageNumber=${currentPage}&pageSize=${pageSize}&categoryId=1`;
