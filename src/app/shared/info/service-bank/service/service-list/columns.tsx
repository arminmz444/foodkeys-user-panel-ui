...
export const getColumns = ({
  ...
  // },
  {
    ...
    key: 'subCategoryName',
    width: 150,
    render: (subCategoryName: string) => <Text>{subCategoryName}</Text>,
  },
  // {
  ...
  },
  {
    ...
    width: 5,
    render: (_: string, row: any) => (
      <Text>{(row.ranking || 0) + ' / ' + (row.rankingAll || 0)}</Text>
    ),
  },
  {
    title: <></>,
    ...
    render: (_: any, row: any) => (
      <div className="flex h-full w-full flex-shrink-0 flex-grow-0 items-center justify-end gap-3 pe-3">
        ...
        >
          <Link
            href={routes.info.serviceIndustryEdit(
              row.subCategoryNameEn,
              row.id
            )}
          >
            <Button
        ...
        <Tooltip
          size="sm"
          content={() =>
            row.hasDetailsPage ? 'مشاهده صفحه اختصاصی' : 'فاقد صفحه اختصاصی'
          }
          placement="top"
          color="invert"
        >
