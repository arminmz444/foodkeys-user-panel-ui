  ...
  if (status === 0) {
    return (
      <>
        ...
        />
        <Text className="pt-1.5 text-[13px] text-gray-500">عدم موجودی</Text>
      </>
    );
  } else if (status <= 20) {
    return (
    ...
    return (
      <>
        ...
        />
        <Text className="pt-1.5 text-[13px] text-gray-500">{status} موجود</Text>
      </>
    );
  }
}
...
export const getColumns = ({
  ...
  },
  {
    ...
    width: 150,
    render: (value: string) => (
      <Text className="font-medium text-gray-700">{value} تومان</Text>
    ),
  },
  {
    title: <HeaderCell title="امتیاز" />,
    dataIndex: 'rating',
    key: 'rating',
    width: 200,
      ...
      <div className="flex items-center justify-end gap-3 pe-4">
        ...
        >
          <Link href={routes.eCommerce.ediProduct(row.id)}>
            <ActionIcon size="sm" variant="outline" aria-label={'ویرایش محصول'}>
        ...
        >
          <Link href={routes.eCommerce.productDetails(row.id)}>
            <ActionIcon size="sm" variant="outline" aria-label={'نمایش محصول'}>
        ...
        <DeletePopover
          title={`حذف محصول`}
          description={`آیا میخواهید این محصول را پاک کنید?`}
